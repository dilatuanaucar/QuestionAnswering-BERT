...
if __name__ == '__main__':
    ...
    query = "<<<Write your answer here>>>"
    source = "source.txt"


    paras_to_search = search_and_format_text(query, source)

    questions, answers, para, probs = answer_prediction(paras_to_search, query,
                                                        'bert-large-uncased-whole-word-masking-finetuned-squad-pytorch_model.bin',
                                                        'bert_config.json')
    questions_final, answers_final, paras_final, probs_final = zip(
        *sorted(zip(questions, answers, para, probs), reverse=True))
